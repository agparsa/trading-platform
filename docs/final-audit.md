...
| 13–14             | Broker panel, dashboard                         | PARTIAL | the firm's book, sessions, money by currency, connections, security centre, IP rules, webhooks, features, developer reference, and a person's devices — revocable by staff, surviving a relaunch, and ending the sessions that handset holds; **not built:** fee schedules, server-side reports, admin threshold alerts, branding — each stated in `broker-panel.md`                                                                                                        |
...
| 48                | API keys                                        | PASS    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 49                | Webhooks                                        | PASS    | signed, retried, disabled after failures, SSRF-vetted, secret shown once; `reconciliation.mismatch` and `security.alert` are produced and delivered, each at a rate chosen so the receiver keeps reading it (`webhooks.md`)                                                                                                                                                                                                                                                 |
| 50–51             | Notifications, sounds                           | PASS    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 52, 57, 61, 83–86 | Terminal UX, responsive, a11y, quality          | PARTIAL | large-order confirmation, risk as a share of equity, account selector, and an automated WCAG 2.1 AA audit of nine screens on every browser run — which found and fixed five unlabelled inputs and a zoom cap on every page; **not covered:** screen-reader paths, the chart without sight, a light theme, and anything needing a person with assistive technology ([accessibility.md](./accessibility.md))                                                                  |
...
- **Device-bound sessions, the gap this matrix named and then closed.** The
  §13–14 row said revoking a device ended no session because sessions were not
  bound to devices, and called binding them "a schema change and a phase of its
  own". It is done: sessions carry the installation that created them, rotation
  copies it from the row it replaces rather than the request, and revoking a
  device — by its owner or by staff — ends the sessions that handset holds. A
  browser still names no installation and is untouched, which is stated rather
  than papered over.
- **§13–14's own "what is not built" list had gone stale.** It still said
  webhooks, IP rules, an admin device view and production API documentation did
  not exist, long after three of the four were built. A document that
  under-reports a platform gets the work done twice, so it is corrected in
  `broker-panel.md` rather than quietly deleted. Reading it also found the real
  gap it was hiding: the platform raised a WARNING when an unknown device
  registered and gave staff no way to see or stop one — an alarm with nothing
  to do about it. There is now a devices section on a person's record, and a
  staff revocation that the handset cannot lift by relaunching, which is what
  it could do before.
- **§49 had two events it offered nobody.** The row said `reconciliation.mismatch`
  and `security.alert` were waiting on their producers, which was true and is
  exactly the sort of thing an audit carries quietly forever. Both are produced
  now — the audit writer emits an alert for a WARNING and nothing quieter, the
  reconciliation sweep emits a mismatch when a finding is raised or reopened
  and not for a recurrence — and the subscribable catalogue is generated from
  the outbox's own list, so a type that is offered is a type something writes.
  Looking for that gap found a real one beside it: the sweep's finding, risk
  event and outbox row were three separate statements, so a crash between them
  would have left the next sweep calling the drift a recurrence and sending
