...
- `pnpm smoke` — 15 checks: envelopes, auth, a full trade round trip, the ledger,
  ...
  confirm it. That last one throws rather than skipping if the login limiter is
...
- `pnpm smoke:web` — 50 checks across 20 routes, in a real browser: every screen
  ...
  own download.
- `pnpm pentest` — 36 attacks attempted against the compiled binary; an attack
