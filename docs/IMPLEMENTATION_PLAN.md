...
against `MetricsService`, so a dashboard cannot chart a metric nobody emits.
Enabling it in production needs `GRAFANA_ADMIN_PASSWORD` set by the operator.
...
not also running the generator.
