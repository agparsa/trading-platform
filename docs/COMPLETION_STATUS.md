...
*never mark UI-only implementation as complete*. The same rule is applied here to
myself.
...
pnpm typecheck        ok
pnpm lint             ok
pnpm test             94 files, 1188 tests, 0 failures
```

33 Prisma models. 59,136 lines of TypeScript in `apps/`, `packages/`, `prisma/`
...
would describe do not exist, and §45 says *do not document features that do not
exist*.
...
risk ✅ WebSocket ✅ notifications ⚠️ (in-app only) audit ✅
...
The trading chain the prompt draws works from the web terminal through to the
WebSocket event. It does not work from a mobile app, because there is no mobile
app, and it does not end in a push notification or a sound, because neither
exists.
