...
| 43       | Web application            | every route opened (`web-routes.test.ts` checks the list against the app directory) in a real browser by `pnpm smoke:web` — which found a sign-in race, a refusal that looked like a hang, and an orphaned permission decorator |
...
| 39       | Security                   | `pnpm pentest` (sixty-odd attacks, each must be refused), AES-256-GCM at rest, no secrets in git history                                                                                                                        |
| 40       | Testing                    | `pnpm test` — over three thousand tests — including PnL, margin, drawdown, exposure, permissions, order validation, push classification, payment idempotency, and row-level security proved by breaking it                      |
